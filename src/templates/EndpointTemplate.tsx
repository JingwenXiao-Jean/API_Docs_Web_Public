...
const EndpointTemplate = observer(({ pageContext }: { pageContext: { id: string } }) => {
    const { id } = pageContext;
    const { documentStore } = useStores();

    const [copied, setCopied] = useState(false);

    // 页面加载时，如果 endpoints 为空，先拉取一次
    useEffect(() => {
        if (documentStore.endpoints.length === 0) {
            documentStore.fetchEndpoints();
        }
    }, [documentStore]);

    const endpoint = documentStore.endpoints.find((ep: any) => ep.id === id);
            ...
            <Box sx={{ width: "100%" }}>
                {Object.entries(data).map(([key, value]) => {
                    if (typeof value === "object" && value !== null) {
                        ...
                        return (
                            <Box key={key} sx={{ py: 1, borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
    ...
    const renderJsonAsBox = (jsonStr: string) => {
        if (!jsonStr || jsonStr.trim() === "") {
            return <Typography sx={{ color: "#94a3b8" }}>No data</Typography>;
        }
        ...
        <Layout>
            <Box sx={{ display: "flex", gap: 4 }}>
                ...
                <Box sx={{ flex: 1, p: 4 }}>
                    ...
                    {/* Request Parameters */}
                    <Typography variant="h6" sx={{ mb: 2 }}>Request Parameters</Typography>
                            ...
                            <TableBody>
                                {Array.isArray(endpoint.parameters) && endpoint.parameters.length > 0 ? (
                                    endpoint.parameters.map((p: any) => (
                                        <TableRow key={p.name} sx={{ cursor: "pointer", "&:hover td": { color: "#38bdf8" } }}>
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={4} sx={{ textAlign: "center", color: "#94a3b8" }}>No parameters</TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
                        </Table>
                    ...
                    {/* Example Request */}
                    <Typography variant="h6" sx={{ mb: 1 }}>Example Request</Typography>
                    {renderJsonAsBox(endpoint.examples?.request || "")}

                    {/* Example Response */}
                    <Typography variant="h6" sx={{ mt: 4, mb: 1 }}>Example Response</Typography>
